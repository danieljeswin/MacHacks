...
from app.models import Classroom, User, Role, Subject
...
def show_classroom(classroom_id=None):
    ...
    if classroom_id:
        ...
        videos = classroom.video.all()
        return render_template('classroom/classroom_detail.html', title=f'Classroom - {classroom.name}', classroom=classroom, videos=videos)
    
    if _role.name == 'Student':
        page = request.args.get('page', 1, type=int)
        
        classrooms = User.query.filter_by(id=current_user.id).first().classroom.paginate(
...
@bp.route('/remove_student_classroom/<int:classroom_id>/<int:student_id>', methods=['GET'])
@login_required
@role_required('Professor')
