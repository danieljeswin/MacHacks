...
from app.text_summarization.others.logging import logger
...
def load_text(args, source_fp, target_fp, device):
    from app.text_summarization.others.tokenization import BertTokenizer
...
def load_text_from_string(args, source_string, device):
    from app.text_summarization.others.tokenization import BertTokenizer
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True)
    sep_vid = tokenizer.vocab['[SEP]']
    cls_vid = tokenizer.vocab['[CLS]']
    n_lines = len(source_string.split('\n'))
